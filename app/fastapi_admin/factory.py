...
from . import enums
from .common import get_all_models, import_obj
...
from ..core.auth.utils.contrib import authenticate_user


async def login(login_in: LoginIn):
    user = await authenticate_user(login_in.username, login_in.password)
    if user:
        await update_last_login(user.id)
    elif not user:
        raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="Inactive user")
    elif not user.is_active:
        raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="Incorrect email or password")
...
class AdminApp(FastAPI):
    ...
    async def init(
        ...
        for model_name, model in get_all_models():
            if issubclass(model, AbstractUser) or model_name == 'User':
